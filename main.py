...
def fitness_evaluate(population, curr_gen):
    ...
    zen_score, num_parameters, flops = fitnessEvaluate(filenames, curr_gen, is_test=False,
                                                       population=population_downscale)
    return zen_score, num_parameters, flops
...
def evolve(adapt_iteration, weight_set, c1_set, c2_set, population, gbest_individual, pbest_individuals, velocity_set,
           params,
           previous_zen_score_set, best_zen_score_set, best_num_parameters, gbest_zen_score, best_flops,
           curr_gen):
    offspring = []
    new_velocity_set = []
    new_weight_set = []
    ...
    for i, particle in enumerate(population):
        new_particle, new_velocity, new_weight, new_c1, new_c2 = aconpso(adapt_iteration, weight_set[i], c1_set[i],
                                                                         ...
                                                                         best_zen_score_set[i], best_zen_score_set,
                                                                         best_num_parameters,
                                                                         gbest_zen_score,
                                                                         best_flops,
                                                                         curr_gen)
...
# 更新个体最佳解和全局最佳解的粒子信息
def update_best_particle(population, best_zen_score_set, num_parameters, flops, gbest, pbest):
    # 生成当前代种群个体最佳解的适应度值集合
    fitnessSet = [
        best_zen_score_set[i] * pow(num_parameters[i] / Tp, wp[int(bool(num_parameters[i] > Tp))]) * pow(flops[i] / Tf,
                                                                                                         wf[
                                                                                                             int(bool(
                                                                                                                 flops[
                                                                                                                     i] > Tf))])
    ...
    if not pbest:
        ...
        pbest_zen_score_Set = copy.deepcopy(best_zen_score_set)
        pbest_params = copy.deepcopy(num_parameters)
        pbest_flops = copy.deepcopy(flops)
        gbest_individual, gbest_zen_score, gbest_params, gbest_flops, gbest_fitness = getGbest(
            [pbest_individuals, pbest_zen_score_Set, pbest_params, pbest_flops])
    # 存在个体最佳解求更新全局最佳解
    else:
        gbest_individual, gbest_zen_score, gbest_params, gbest_flops = gbest
        pbest_individuals, pbest_zen_score_Set, pbest_params, pbest_flops = pbest

        # 当前代种群的个体最佳解集合
        pbest_fitnessSet = [
            pbest_zen_score_Set[i] * pow(pbest_params[i] / Tp, wp[int(bool(pbest_params[i] > Tp))]) * pow(
                pbest_flops[i] / Tf, wf[int(bool(pbest_flops[i] > Tf))]) for i in range(len(pbest_individuals))]

        # 计算当前的全局最佳解的适应度值
        gbest_fitness = gbest_zen_score * pow(gbest_params / Tp, wp[int(bool(gbest_params > Tp))]) * pow(
        ...
        for i, fitness in enumerate(fitnessSet):
            if fitness > pbest_fitnessSet[i]:
                ...
                pbest_zen_score_Set[i] = copy.deepcopy(best_zen_score_set[i])
            ...
            if fitness > gbest_fitness:
                ...
                gbest_zen_score = copy.deepcopy(best_zen_score_set[i])
                gbest_params = copy.deepcopy(num_parameters[i])
                gbest_flops = copy.deepcopy(flops[i])

    return [gbest_individual, gbest_zen_score, gbest_params, gbest_flops], [pbest_individuals, pbest_zen_score_Set,
                                                                            pbest_params,
                                                                            pbest_flops]
...
def getGbest(pbest):
    pbest_individuals, pbest_zen_score_Set, pbest_params, pbest_flops = pbest
    gbest_zen_score = float('-inf')
    gbest_params = 10e6
    gbest_flops = 10e9
    gbest = None

    # 初始化全局最佳解
    gbest_fitness = gbest_zen_score * pow(gbest_params / Tp, wp[int(bool(gbest_params > Tp))]) * pow(gbest_flops / Tf,
                                                                                                     wf[int(bool(
                                                                                                         gbest_flops > Tf))])

    # 遍历粒子的每个个体最佳解得到一个个体最佳解的集合
    pbest_fitnessSet = [pbest_zen_score_Set[i] * pow(pbest_params[i] / Tp, wp[int(bool(pbest_params[i] > Tp))]) * pow(
        pbest_flops[i] / Tf, wf[int(bool(pbest_flops[i] > Tf))]) for i in range(len(pbest_individuals))]
    ...
    for i, indi in enumerate(pbest_individuals):
        ...
        if pbest_fitnessSet[i] > gbest_fitness:
            ...
            gbest_zen_score = copy.deepcopy(pbest_zen_score_Set[i])
    ...
    return gbest, gbest_zen_score, gbest_params, gbest_flops, gbest_fitness
...
def fitness_test(final_individual):
    ...
    zen_scores, num_parameters, flops = fitnessEvaluate([filename], -1, True, [final_individual], [batch_size],
                                                        [weight_decay])
    return zen_scores[0], num_parameters[0], flops[0]


def calculate_fitness(zen_score, num_parameters, flops):
    zen_score_set_mean = np.mean(zen_score)
    num_parameters_mean = np.mean(num_parameters)
    flops_mean = np.mean(flops)
    weighted_avg = zen_score_set_mean * 0.7 - num_parameters_mean / 1000000 * 0.15 - flops_mean / 1000000 * 0.15
    return weighted_avg
...
def evolveCNN(params):
    ...
    for i in range(0, start_init_gen):
        ...
        zen_score, num_parameters, flops = fitness_evaluate(population, -(i + 1))
        Log.info('第 %d 个种群的平均zen_score：' % i + ':' + str(np.mean(zen_score)))
        current_weight_avg = calculate_fitness(zen_score, num_parameters,
                                               flops)
        if current_weight_avg > weight_avg:
            weight_avg = current_weight_avg
            best_zen_score_set = zen_score
            best_num_parameters = num_parameters
            best_flops = flops
            best_population = population

    Log.info('初始化得到的最佳polulation：' + str(best_population))
    Log.info('初始化第一代运行的最佳架构zen_score均值' + str(np.mean(best_zen_score_set)))
    Log.info('EVOLVE[%d-gen]-当前代粒子群更新初始化的最佳加权适应度[ %d ],最佳初始化zen_score[ %.4f ]' % (
        gen_no, weight_avg, np.mean(best_zen_score_set)))
    previous_zen_score_set = best_zen_score_set
    ...
    [gbest_individual, gbest_zen_score, gbest_params, gbest_flops], [pbest_individuals, pbest_zen_score_Set,
                                                                     pbest_params,
        ...
        gbest=None,
        pbest=None)
    Log.info('第一代的gbest_zen_score' + str(gbest_zen_score))
    Log.info('第一代的pbest_zen_score_Set' + str(np.mean(pbest_zen_score_Set)))
    Log.info('EVOLVE[%d-gen]-Finish the updating（初始化第一代的个体最佳解、全局最佳解完成）' % (gen_no))

    # 第一代的种群根据当前的种群 错误率 参数 浮点运算数 代数进行结果保存
    Utils.save_population_and_zen_score('populations', best_population, best_zen_score_set, best_num_parameters,
                                        best_flops, gen_no)
    Utils.save_population_and_zen_score('pbest', pbest_individuals, pbest_zen_score_Set, pbest_params, pbest_flops,
                                        gen_no)
    Utils.save_population_and_zen_score('gbest', [gbest_individual], [gbest_zen_score], [gbest_params], [gbest_flops],
                                        gen_no)
    ...
    for curr_gen in range(gen_no, params['num_iteration']):
        ...
        Log.info('前一代架构运行的zen_score' + str(np.mean(previous_zen_score_set)))
        Log.info('后一代及以后的架构运行的zen_score' + str(np.mean(best_zen_score_set)))
        ...
        best_population, velocity_set, weight_set, c1_set, c2_set = evolve(adapt_iteration, weight_set, c1_set, c2_set,
                                                                           ...
                                                                           params,
                                                                           previous_zen_score_set,
                                                                           best_zen_score_set, best_num_parameters,
                                                                           gbest_zen_score,
                                                                           best_flops,
                                                                           curr_gen)
        ...
        previous_zen_score_set = best_zen_score_set
        # 适应度评估
        best_zen_score_set, best_num_parameters, best_flops = fitness_evaluate(best_population, curr_gen)
        ...
        [gbest_individual, gbest_zen_score, gbest_params, gbest_flops], [pbest_individuals, pbest_zen_score_Set,
            ...
            pbest=[
                ...
                pbest_params,
                pbest_flops])
        Log.info('第%d代的gbest_zen_score' % (curr_gen) + str(gbest_zen_score))
        Log.info('第%d代的pbest_zen_score_Set' % (curr_gen) + str(np.mean(pbest_zen_score_Set)))
        ...
        Utils.save_population_and_zen_score('pbest', pbest_individuals, pbest_zen_score_Set, pbest_params, pbest_flops,
                                            curr_gen)
        Utils.save_population_and_zen_score('gbest', [gbest_individual], [gbest_zen_score], [gbest_params],
    ...
    proxy_err = copy.deepcopy(gbest_zen_score)

    # final training and test on testset
    gbest_zen_score, num_parameters, flops = fitness_test(gbest_individual)
    Log.info('Error=[%.5f], #parameters=[%d], FLOPs=[%d]' % (gbest_zen_score, gbest_params, gbest_flops))
    Utils.save_population_and_zen_score('final_gbest', [gbest_individual], [gbest_zen_score], [num_parameters], [flops],
                                        -1,
