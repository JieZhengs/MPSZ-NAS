...
import load_dataset.data_loader_road as data_loader
import os
import numpy as np
import math
import time
...
class TrainModel(object):
    def __init__(self, is_test, particle, batch_size, weight_decay):
        if is_test:
            ...
            full_trainloader = data_loader.get_train_loader('../datasets/road_data/train', batch_size=batch_size,
                                                            augment=True, shuffle=True, num_workers=4, pin_memory=True)
            testloader = data_loader.get_test_loader('../datasets/road_data/val', batch_size=batch_size,
                                                     shuffle=False,
    ...
    def train(self):
        ...
        info = compute_nas_score(model=self.net, resolution=256, batch_size=16)
    ...
    def test(self, epoch):
        with torch.no_grad():
            ...
            for _, data in enumerate(testloader, 0):
                ...
                testloader.desc = "[Test epoch {}]  Loss {} Acc {} ".format(epoch + 1, test_loss / total,
                                                                            correct / total)
