...
import load_dataset.data_loader_RTK as data_loader
import os
import numpy as np
import math
import time
...
class TrainModel(object):
    def __init__(self, is_test, particle, batch_size, weight_decay):
        if is_test:
            ...
            full_trainloader = data_loader.get_train_loader('../datasets/RTK_data/train', batch_size=batch_size,
                                                            augment=True, shuffle=True, num_workers=4, pin_memory=True)
            testloader = data_loader.get_test_loader('../datasets/RTK_data/val', batch_size=batch_size,
                                                     shuffle=False,
    ...
    def test(self, epoch):
        ...
        if correct / total > self.best_acc:
            torch.save(self.net.state_dict(), './trained_models/RQ_CNN.pth')
